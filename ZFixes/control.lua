...
script.on_configuration_changed(function (data)
  for _, force in pairs(game.forces) do
    if force.name ~= "enemy" and force.name ~= "neutral" then
      -- Friendly fire
      force.friendly_fire = settings.global["ZFixes-Friendly-Fire"].value
      -- Friendly Fire
    end
  end
...
script.on_event(defines.events.on_player_died, function(event)
  local respawnTime = settings.global["ZFixes-Respawn"].value
  game.players[event.player_index].ticks_to_respawn = respawnTime * 60
end)
